%% NGA-West2 RotD100 Exceedance Analysis (MCER Level)
% Identifies and records ground motions that exceed the ASCE 7-22 MCER spectra.
...
% Set up storage for results in data folder
resultFile = fullfile(data_dir, 'MCE_Results.mat');
...
for j = 1:length(rsn_gm)
    ...
    if isprop(storedResults, sprintf('twoT_MCE_T_%d', rsn_val))
        two_T_MCE_T = storedResults.(sprintf('twoT_MCE_T_%d', rsn_val));
        two_T_MCE_ord = storedResults.(sprintf('twoT_MCE_SA_%d', rsn_val));
        multi_T_MCE_T = storedResults.(sprintf('multiT_MCE_T_%d', rsn_val));
        multi_T_MCE_ord = storedResults.(sprintf('multiT_MCE_SA_%d', rsn_val));
    else
        % Generate site-specific ASCE 7-22 spectrum (calls function in src)
        ...
        storedResults.(sprintf('twoT_MCE_T_%d', rsn_val)) = two_T_MCE_T;
        storedResults.(sprintf('twoT_MCE_SA_%d', rsn_val)) = two_T_MCE_ord;
        storedResults.(sprintf('multiT_MCE_T_%d', rsn_val)) = multi_T_MCE_T;
        storedResults.(sprintf('multiT_MCE_SA_%d', rsn_val)) = multi_T_MCE_ord;
    end
    ...
    if any(Sa_RotD100 > 0)
        ...
        T_comp_1 = two_T_MCE_T;
        Sa_comp_1 = two_T_MCE_ord;
        T_comp_2 = multi_T_MCE_T;
        Sa_comp_2 = multi_T_MCE_ord;
...
save(fullfile(data_dir,"rsn_exceed_NGA_West_MCE.mat"), ...
    "rsn_great_1","not_found_RSN","rsn_great_2");
save(fullfile(data_dir,"val_exceed_NGA_West_MCE.mat"), ...
    "T_great_2", "ratio_dif_SA_2","mult_dif_SA_2", ...
    "T_great_1", "ratio_dif_SA_1","mult_dif_SA_1");
save(fullfile(data_dir,"T_vs_RSN_MCE.mat"), ...
    "T_uni_great_2","T_uni_great_1");
save(fullfile(data_dir,"intervals_exceed_MCE.mat"), ...
    "t_int_2","t_int_1");

disp('Finished MCER exceedance analysis.');